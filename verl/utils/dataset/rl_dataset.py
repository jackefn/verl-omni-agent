...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            if self.enable_audio and self.config.get("use_audio_in_video", False) and row_dict_videos:
                    audios_from_videos = [
                        process_audio(
                            video["video"][len("file://"):] if isinstance(video, dict) and video.get("video", "").startswith("file://") 
                            else video["video"] if isinstance(video, dict) 
                            else video, 
                            True
                        ) 
                    ...
                    multi_modal_data["audio"] = [
                        audio if isinstance(audio, np.ndarray) else audio.cpu().numpy()
                        for audio in audios
                    ]
            # model_inputs = self.processor(text=[raw_prompt], images=images, videos=videos, return_tensors="pt")
            model_inputs = self.processor(
                text=[raw_prompt], images=None, videos=videos, audio=audios, return_tensors="pt"
            )
