...
class DummyVideoRetrievalEnv:
    """
    单环境版本的底层环境，负责真正的step/reset逻辑。
    """
    def __init__(self):
        self.t = 0
    ...
    def reset(self, kwargs=None):
        ...
        video = np.zeros((8, 224, 224, 3), dtype=np.uint8)
        audio = np.zeros((16000,), dtype=np.float32)
        info = {"dummy_info": True}
        return img, video, audio, info

    def step(self, action):
        self.t += 1
        img = np.ones((224, 224, 3), dtype=np.uint8) * self.t
        video = np.ones((8, 224, 224, 3), dtype=np.uint8) * self.t
        audio = np.ones((16000,), dtype=np.float32) * (self.t / 10.0)
        reward = float(self.t)
        done = self.t >= 3
        info = {"time": self.t}
        return img, video, audio, reward, done, info

class DummyVideoWorker:
    """
    单环境 Worker，用于 Ray actor。
    """
    def __init__(self):
        from copy import deepcopy
        self.env = DummyVideoRetrievalEnv()

    def reset(self, kwargs=None):
        img, video, audio, info = self.env.reset(kwargs)
        return img, video, audio, info

    def step(self, action):
        img, video, audio, reward, done, info = self.env.step(action)
        return img, video, audio, reward, done, info

class VideoRetrievalEnvironmentManager(EnvironmentManagerBase):
    """
    管理多个 DummyVideoRetrievalEnv，使用 Ray 多进程封装，
    支持 GRPO/GiGPO 批量 rollout。
    """
    def __init__(self, num_envs=4, resources_per_worker=None, projection_f=None, config=None):
        super().__init__(None, projection_f, config)  # 先调用基类构造函数
        ...
        # 封装 worker
        worker_cls = ray.remote(**resources_per_worker)(DummyVideoWorker)
        self.workers = [worker_cls.remote() for _ in range(num_envs)]

    def reset(self, kwargs=None):
        futures = [w.reset.remote(kwargs) for w in self.workers]
        results = ray.get(futures)

        imgs, videos, audios, infos = zip(*results)
        observations = {
            "text": ["" for _ in range(self.num_envs)],
            "image": list(imgs),
            "video": list(videos),
            "audio": list(audios)
        }
        return observations, list(infos)

    def step(self, text_actions):
        assert len(text_actions) == self.num_envs, f"actions length {len(text_actions)} != num_envs {self.num_envs}"
        futures = [w.step.remote(a) for w, a in zip(self.workers, text_actions)]
        results = ray.get(futures)

        imgs, videos, audios, rewards, dones, infos = zip(*results)
        observations = {
            "text": ["" for _ in range(self.num_envs)],
            "image": list(imgs),
            "video": list(videos),
        ...
        return observations, rewards, dones, list(infos)
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "video_retrieval" in config.env.env_name.lower():
        num_envs = config.env.rollout.n
        projection_f = None 
        envs = VideoRetrievalEnvironmentManager(num_envs, projection_f, config)
        val_envs = VideoRetrievalEnvironmentManager(num_envs, projection_f, config)
        return envs, val_envs
    else:
